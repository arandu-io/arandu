...
type Deps struct {
	...
	// nothing, and widening Run would put an ignored parameter on every one of
	// them forever. A seeder that reads flags says so by reading this field.
	Args []string
}
...
// is two spellings of one thing, so the flag form `--class=` is refused with the
// word to use instead rather than accepted quietly.
//
